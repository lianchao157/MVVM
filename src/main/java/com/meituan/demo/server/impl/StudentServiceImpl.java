...
import com.meituan.demo.server.StudentService;
...
public class StudentServiceImpl implements StudentService {
    @Autowired
    private MeiTuan_ShopCarMapepr vlm;
//    selectallshopcar
    ...
    public Integer importExcel(MultipartFile myFile) throws Exception {
        ...
        for (int i = 1; i <= rows+1; i++) {
            ...
            if (row != null) {
                // **读取cell**
//                MeiTuan_ShopCar_Bean student = new MeiTuan_ShopCar_Bean();
//                //姓名
...
//        style.setAlignment(HSSFCellStyle.ALIGN_CENTER); // 创建一个居中格式
        ...
        List<MeiTuan_ShopCar_Bean> list = vlm.selectallshopcar();

        for (int i = 0; i < list.size(); i++){
            row = sheet.createRow(i + 1);
            MeiTuan_ShopCar_Bean stu = list.get(i);
            // 第四步，创建单元格，并设置值
            row.createCell(0).setCellValue(stu.getColumn_11());
            row.createCell(1).setCellValue(stu.getShopcar_id());
            row.createCell(2).setCellValue(stu.getShopcarname());
            cell = row.createCell(3);
